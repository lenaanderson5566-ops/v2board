function buildClientStrategySummary(rows) {
  ...
  return metrics.length ? `<div class="cards" style="margin-bottom:12px;">${metrics.map(item => `<div class="card"><div class="label">${item.label}</div><div class="value">${item.value}</div></div>`).join('')}</div>` : '';
}

function renderClientStrategyOverview(rows) {
  if (!rows || !rows.length) return document.getElementById('result').innerHTML = '<p>暂无客户端策略数据</p>';
  document.getElementById('result').innerHTML = buildClientStrategySummary(rows) + buildTable(rows);
}
...
async function fetchClientStrategyOverview(btn) { setView(btn, '客户端策略总览'); const rows = await request('/client/strategy/fetch'); if (rows) renderClientStrategyOverview(rows); }
